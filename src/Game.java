    ...
    private void initialize(File inputFile) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < M; i++) {
                ...
                if (line.length() != N) {
                    map = null;
                    throw new Exception("Corrupted map definition");
                }
                ...
                for (int j = 0; j < line.length(); ++j) {
                    ...
                    // Validate character
                    if (!validMapObjects.contains(ch)) {
                        map = null;
                        throw new Exception("Corrupted map definition");
                    }
            ...
            for (int i = 0; i < numOfPoke; ++i) {
                ...
                if (parts.length != 6) {
                    map = null;
                    throw new Exception("Corrupted map definition. Pokemon definition: Expected 6 chunks of information.");
                }
                ...
                if (!(map.getCell(row, col) instanceof Pokemon)) {
                    map = null;
                    throw new Exception("Expected Pokemon object type at: " + row + "\t" + col + "\t" + this.map.getCell(row, col).getClass());
                }
                // Get pokemon cell
            ...
            for (int i = 0; i < numOfStat; ++i) {
                ...
                if (parts.length != 3) {
                    map = null;
                    throw new Exception("Corrupted map definition. Station definition: Expected 3 chunks of information.");
                }

                // Parse row/col number
                int row = Integer.parseInt(parts[0].substring(parts[0].indexOf("<") + 1).trim());
                int col = Integer.parseInt(parts[1].substring(0, parts[1].indexOf(">")).trim());
                if (!(this.map.getCell(row, col) instanceof Station)) {
                    map = null;
                    throw new Exception("Expected Station object type at: " + row + "\t" + col + "\t" + this.map.getCell(row, col).getClass());
                }
